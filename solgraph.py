...
Return one word: \"yes\" or \"no\", nothing else. I forbid you from thinking too much or analyzing the solutions too much.""",
    ...
    def addSolution(self, solution_uid, solution_text, is_correct):
        ...
        # Inline step-size control: if config.json step_size == "small" use granular steps; else use big steps
        step_size = "big"
        try:
            cfg_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "config.json"))
            with open(cfg_path, "r", encoding="utf-8") as _f:
                _cfg = json.load(_f)
                step_size = str(_cfg.get("step_size", "big")).strip().lower()
        except Exception:
            pass

        if step_size in ("small", "smol"):
            sys_prompt = r'''You are a solution explainer.

Your task is to take a complete solution and reformat it into small, granular steps. Split whenever there is a nontrivial algebraic manipulation, substitution, inference, or change of idea. Do not add new reasoning or solve the problem yourself — just restructure what is already there.  
